...
export function StaffSidebar() {
  ...
  // Add CRM for marketers and customer care
  if (userData?.role === 'marketer_adhoc' || userData?.role === 'marketer_contract' || userData?.role === 'customer_care') {
    navigation.push({ name: 'CRM / Leads', href: '/crm', icon: Briefcase });
  }

  // Add Department Overview for department heads
  if (userData?.role === 'department_head') {
    navigation.push({ name: 'Department Overview', href: '/department', icon: Briefcase });
  }
    ...
    <div className="flex flex-col w-64 bg-white border-r border-slate-200 min-h-screen">
      <div className="flex items-center justify-center h-20 border-b border-slate-100 px-6">
        <div className="flex items-center space-x-3 w-full">
          <div className="h-10 w-10 bg-[#F5A623] rounded-xl flex items-center justify-center shadow-sm shrink-0">
            <span className="text-white font-bold tracking-tight text-sm">ASB</span>
          </div>
          <span className="font-bold text-slate-800 text-lg whitespace-nowrap overflow-hidden text-ellipsis">Staff Portal</span>
        </div>
      </div>
            ...
            return (
              <Link
                ...
                className={`
                  ...
                  ${isActive 
                    ? 'bg-orange-50 text-[#d97706] shadow-sm border border-orange-100/50' 
                    : 'text-slate-600 hover:bg-slate-50 hover:text-slate-900'}
                `}
              >
                <item.icon 
                  className={`flex-shrink-0 mr-3 h-5 w-5 ${isActive ? 'text-[#d97706]' : 'text-slate-400'}`} 
                />
                {item.name}
              </Link>
            );
